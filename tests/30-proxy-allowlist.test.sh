...
start_proxy() { # start_proxy <name> [allowlist-file]
  ...
  for i in $(seq 1 60); do
    [ -n "$(http_code "http://readonly-api:8080/ping" --proxy "http://$name:8080")" ] && \
      [ "$(http_code "http://readonly-api:8080/ping" --proxy "http://$name:8080")" != "000" ] && \
      { echo "$name"; return 0; }
    sleep 0.5
  done
  echo "$name"
...
if PX=$(start_proxy permissive); then
...
if PXA=$(start_proxy enforcing "$FIXTURES/allowlist"); then
