#!/usr/bin/env bash
# Integration suite runner. Usually reached through the tests/run.sh facade,
# but runnable directly and takes the same arguments either way.
#
#   tests/integration/run.sh            # everything
#   tests/integration/run.sh 00 10      # only suites starting with 00 or 10
#   FORCE_BUILD=1 tests/integration/run.sh   # rebuild images, don't reuse
#
# Exit code is non-zero if any suite fails. No credentials are required: every
# suite runs against stubs and fixtures. For the real-credential tier see
# tests/e2e/.
set -uo pipefail
